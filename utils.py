...
def resize_only(Is, inc):
	rev_axis = False
	if len(Is.shape) == 3:
		rev_axis = True
		Is = Is[:,:,:,np.newaxis]
	imr = np.zeros((Is.shape[0]*inc, Is.shape[1]*inc, Is.shape[2], Is.shape[3]))
	for kk in range(Is.shape[3]):
		im = Is[:,:,:,kk]
		imr[:,:,:,kk] = cv2.resize(im, [imr.shape[1],imr.shape[0]], interpolation = cv2.INTER_CUBIC)
	if rev_axis:
		imr = imr[:,:,:,0]
	return imr
